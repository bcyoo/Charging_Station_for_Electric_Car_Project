...
#
# **분석 설명**
# - 빨간색일 수록 인구가 많이 있으며, 색이 옅어질수록 인구가 적은 것이다. 색이 칠해지지 않은 곳은 인구 현황 값이 0 이다.
...
layer = pdk.Layer( 'PolygonLayer', # 사용할 Layer 타입 
                  ...
                  get_fill_color='[255, 255*정규화인구, 0, 정규화인구*10000 ]', # 각 데이터 별 rgb 또는 rgba 값 (0~255)
...
#
# **분석 데이터 종류**
# - df_03: "03.속초시-고성군_자동차등록현황.csv", encoding='utf-8'
...
# ### 분석 결과
# - 속초시 조양동과 교동에 자동차 등록이 많이 됨
# - 고성군은 토성면과 간성읍에 자동차 등록이 많이 됨
# - 자동차 등록 위치는 세대 위치와 비슷하므로 자동차 등록대수가 많은 곳에 완속충전소를 세우는 것이 효과적일것으로 판단
# - 속초시 조양동과 교동, 고성군 토성면과 간성읍에 완속충전소를 설치하는 것이 효과적일 것으로 기대
...
#
# **분석 데이터 종류**
# - df_06: 06.속초시-고성군_전기자동차등록현황.csv
# - df_20: 20.행정경계(읍면동).geojson
#
# **분석 설명**
...
# ### 분석 결과
# - 속초시 조양동과 교동에 '전기' 자동차 등록이 많이 됨
# - 고성군은 토성면에 자동차 등록이 많이 됨
# - 자동차 등록 위치는 세대 위치와 비슷하므로 자동차 등록대수가 많은 곳에 완속충전소를 세우는 것이 효과적일것으로 판단
# - 속초시 조양동과 교동, 고성군 토성면에 완속충전소를 설치하는 것이 효과적일 것으로 기대
#
...
r.to_html('교통량_17.html')

# ### 분석 결과
# - 퇴근 시간을 피한 오후 17시에 교통량이 많은 곳은 관광객이 많이 가는 지역일 것으로 기대
...
for i in tqdm(range(len(df_03))):
    ADM_points.append([df_03.loc[i,'EMD_NM'],
                       df_03.loc[i,'2021'],
                        point_cent.buffer(0.00000001).within(df_03.loc[i,'geometry'])])
df_result['자동차등록'] = 0

for i in range(len(ADM_points)):
    df_result['자동차등록'][ADM_points[i][2]] = ADM_points[i][1]
df_result

## 자동차등록 17개 중 2381개가 point와 관련되어있는 grid 개수이고
## 임의 gird 중 71581개 중 자동차등록 17개가 개발가능여부
# -
...
#
# **분석 설명**
# - 각 지역에 해당하는 Point를 모두 추출, 전기자동차 보급 현황을 부여하였다.
...
## 전기자동차등록 17개 중 2381개가 point와 관련되어있는 grid 개수이고
## 임의 gird 중 71581개 중 교통량 5754개가 개발가능여부
# -

# ## 2. 100X100 Point에 교통량, 혼잡빈도강도, 혼잡시간강도, 자동차등록, 전기자동차등록 관련 요소 부여
#
# **목적: grid 마다 교통량 관련 요소 부여**
#
# **분석 데이터 종류**
# - df_03: 03.속초시-고성군_자동차등록현황.csv
# - df_06: 06.속초시-고성군_전기자동차등록현황.csv
...
#
# **분석 설명**
# - 각 100X100 Point 마다 11시 교통량, 17시 교통량, 혼잡빈도강도합, 혼잡시간강도합, 자동차등록, 전기자동차등록을 부여
...
df_LR = df_result
X = df_LR[['정규화_인구',"정규화_교통량_11","정규화_교통량_17","정규화_혼잡빈도강도합","정규화_혼잡시간강도합",'정규화_자동차등록']]
...
X = [0.43968359, 0.1848299, -0.34327869, -1.15949403, 1.6665829, -0.02913139, 0.0308937]
y = [1.35365108, -0.1610098, 0.04818647, -0.22488084, 1.48347408, -0.14921298, 0.24322933]
plt.figure(figsize=(15,10))
plt.plot(X)
plt.plot(y)
plt.title("선형회귀분석 coefficient")
plt.xticks([0,1,2,3,4,5,6], ['정규화_인구', '정규화_교통량_11', '정규화_교통량_17', '정규화_혼잡빈도강도합', '정규화_혼잡시간강도합', '정규화_자동차등록', '정규화_전기자동차등록'], rotation=20)
...
try:    
    df_result[['grid_id','geometry',
               ...
               '정규화_혼잡빈도강도합', '정규화_혼잡시간강도합', '정규화_자동차등록',
...
opt2 = pdk.Layer(
    ...
    pickable=True)
# Render
r = pdk.Deck(layers=[layer3, opt1,opt2], initial_view_state=view_state)
            # mapbox_key = "pk.eyJ1IjoiamNsYXJhODExIiwiYSI6ImNrZzF4bWNhdTBpNnEydG54dGpxNDEwajAifQ.XWxOKQ-2HqFBVBYa-XoS-g")
r.to_html('최종 개발 가능 지역_1.html')
# -
